...
| `DELETE` | `/api/classes/{id}` | Оператор+ | Удалить класс (ученики открепляются, попадают в history) |
| `PATCH` | `/api/classes/{id}/teacher` | Оператор+ | Назначить классного руководителя (только роль «Учитель») |
| `DELETE` | `/api/classes/{id}/teacher` | Оператор+ | Снять классного руководителя |
| `GET` | `/api/classes/{id}/students` | Классный рук. / Оператор+ | Список учеников класса с группами |
...
| `DELETE` | `/api/classes/{id}/students/{user_id}` | Классный рук. / Оператор+ | Исключить ученика из класса |
...
Обычный учитель без класса не может приглашать и управлять контингентом.
...
| Метод | Путь | Доступ | Описание |
| --- | --- | --- | --- |
| `GET` | `/api/seating/health` | Публичный | Статус сервиса рассадки |
