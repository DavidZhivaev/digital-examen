...
def add_bottom_3blocks(c: CanvasType, y_top: float, data: PersonData) -> float:
    ...
    c.drawCentredString(WIDTH / 2, 45, "Подпись ответветственного организатора строго в окошке")
...
def add_exam_lists_grid(c: CanvasType, y_top: float) -> float:
    ...
    text_y = y_top - 18
    c.drawCentredString(WIDTH / 2, text_y, "Решение тестовой части. Каждый символ СТРОГО в клеточке!")
    ...
    c.setFont("Spectral-Bold", 12)
    text_y = line_y - 18
    c.drawCentredString(WIDTH / 2, text_y, "Замена ошибочных ответов. Каждый символ СТРОГО в клеточке!")
    ...
    text_y = line_y - 18
    c.drawCentredString(WIDTH / 2, text_y, "ЗАПОЛНЯЕТСЯ ОТВЕТСТВЕННЫМ ОРГАНИЗАТОРОМ В АУДИТОРИИ:")
